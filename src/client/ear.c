 ...
 * main thread.
 */
int alloc_thread(int port, char ipv6) {
...
    
    ...
    if(thread_root == NULL) {
        ...
        process_error(ERROR_MAJOR, "Error allocating thread structure", errno);
        return(1);
    }
...
        /*
         ...
         * message queue when there's a directed attack or a complex scan.
         */
...
    for(;;) {
        ...
        /*
         * Do the data reduction checks.  This reduces events placed on the
         * mssage queue when there's a directed attack or a complex scan.
         */
